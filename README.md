...
The 3705 emulator should run on any Linux environment.  However, testing has only be done on Raspberry Pi and Intel platforms.
...
```
1. i3705	This is the actual 3705 emulator. 	**Build instr: $ make i3705 ** 
2. i3274	The 3274 PU type 2 emulator (SDLC). 	**Build instr: $ make i3274 **
3. i3271	The 3271 cluster emulator (BSC). 	**Build instr: $ make i3271 **
4. DLSw	Data Link Switch (SDLC).			**Build instr: $ make DLSw **
5. Trunk	Interconnection between two 3705's.	**Build instr: $ make Trunk **
6. CPanel	The 3705 Control panel.			**Build instr: $ make CPanel **
```
...
### Starting the 3705
The 3705 may be started before of after Hercules is started. The (TCP/IP) channel connection is (re)established as soon as both ends are available. As such, the 3705 can be stopped and started at any point without the need to restart Hercules.
...
This section is generic. It applies to a CBIPO build of MVS, TK5 or whatever other ways the MVS system has been created.    
...
For TK5 the address range 240-24F are 3350 DASDs.  Below 244 is used.
...
...
| 3705 Channel Adapter | Channel Switch | IP Port  |
| -------------------- | -------------- | ---------|
|          1           |   A position   |   37501  |
|          1           |   B position   |   37503  |
|          2           |   A position   |   37504  |
...
The 3705 can handle 2 channel adapters, each consisting of two channels "A" and "B".    
...
Job NPGEN will create the stage1 deck in dataset SYS1.NCPSTG1 on volume NCPSSP  

Edit this member and go to the last step in the deck (is either step s16 or s17). Change the DISP field of the SYSLMOD statement to DISP=SHR (otherwise VTAM has to be stopped to allow the job to allocate the dataset).   
...
The error indicators are listed separately at the top left (box CCU CHECKS).  If an error occurs, a red 'light' w# IBM3705_R5
## About the 3705 emulator project
This project aims at delivering an IBM 3705-II emulator, specifically designed to connect to the Hercules mainframe emulator.  
The 3705 emulator allows running unmodified IBM NCP software up to NCP version 3 (The last NCP version supporting a 3705-II). The package also includes a 3274 PU T2 emulator as well as a 3271 cluster emulator. This enables 3270 emulation software, like x3270 or tn3270,  to connect to the 3705 using the SLDC or BSC protocol. In addtion, it is even possible to connect actual IBM hardware to the 3705 emulator using a Data Link Switch (DLSw). 
...
In the center of the panel the HEX switches are show. They are labeled A -> E. The actual switches are depicted as single digits. A switch can be selected by the left or right cursor keys. The selected switch will be highlighted. The value can be changed with the up and down cursor keys.   

Below the HEX switches, the DISPLAY FUNCYION SELECT switch is shown, with 10 possible positions. The default is STATUS. The switch can be 'turned' by pressing PF 3 key. The switch turns clockwise. The DISPLAY FUNCTION switch is described in more detail below.   
...
Warning: Switching a channel adapter is immediate. If the (3705) unit is still online while switching, various I/O related errors will occur. An IPL might be needed to recover from this situation. So before switching, make sure the unit is offline on MVS.

The DISPLAY FUNCTION SELECT:
The position of the switch is changed by pressing PF 3. The current selection is highlighted. 
The selections are:   
...
The LOAD switch is used to reboot the 3705. After pressing PF4 the NCP will be reloaded. When -both- channel adapters are fully disabled (so A and B are disabled on both channel adapters) the 3705 will load the remote program loader programs from the (virtual) diskette drive.
The below image shows the Control Panel channel section for a remote 3705:  
  
![LIB panel](/Images/CAs.png)   

The Control panel is updated after pressing any alpha-numeric key, except the Home key.  

Exiting the Control panel: Press the Home key.  

### LIB panel
The Line Interface Base (LIB) is the one place where all lines connect to. Each stand-alone emulator (3271, 3274, DLSw, Trunk) has the -line command parameter as a mandatory start parameter to identify the line to which the connection should be made. Node that this line number must correspond to the line number in the NCP definition for the device being connected.   

A successful connection is identified by the message 'LIB: 327x connected to line-xx', whew xx is the NCP line number.  
...
The above display shows Line 20, connected, active to VTAM and in session (RS232 RTS is high).  
Line 21 is active for VTAM (RS232 DTR high), but not connected. Therefore, this must be a switched line.  
...
```
Connect your TN3270 client to the 3271's IP address with port 32711 (or 32712 for the 2nd cluster).  
You should shee the NETSOL welcome message.  
Logon to TSO:  
Logon HERC01  
press ENTER 

NB: 3270 emulatures allow to specify the LU name. These allow to connect to a specific terminal. So, if you want to connect to the 2nd terminal, specify LU 01 as the terminal name in the 3271 emulator.   
...
The real DLSw route needs to be configured properly. 

The config parameters relevant to the DLSw connection are (for a Cisco 2800 router):  
| Configuration Parameter              | Description                               |
|--------------------------------------|-------------------------------------------|
| dlsw local-peer peer-id 192.168.2.91 | This is the IP address of the DLSw router |
| dlsw remote-peer 0 tcp 192.168.2.72  | The IP addr of the host running the DLSw
|                                      | emulator 
| interface FastEthernet0/0            | Ethernet interface to be used |
| ip address 192.168.2.91 255.255.255.0 | IP addr & netmask of the ethernet 
|                                      | interface.
| interface Serial0/0/0                | Serial interface to which the SDLC device
|                                      | is attached.
| encapsulation sdlc                   | SDLC frame encapsulation.
| clock rate 9600                      | Baud rate between the DSLw router.
| sdlc role primary                    | DLSw router plays role of primary SDLC 
|                                      | device.
| sdlc vmac 4000.0999.0100             | Arbitrary (virtual) MAC addr associated 
|                                      | with the SDLC device. Not used.|
| sdlc address C1                      | Addr of SDLC PU connected to the DLSw router 
| sdlc xid C1 01700018                 | XID of the SDLC PU. Must match VTAM 
|                                      | switched node definition. |
...
This connects one end of Trunk to line 20 and the other end to 21 of i3705 on host efoxcc1, the other line is connected to line 20 and line 21 of i3705 on host efoxcc3.  
...
In a full duplex connection the 2nd line is always consecutive to the specified line. E.g. if the -line1 switch specifies 20 (Tx leg), line 21 (Rx leg) is implicitly used as well.  
...
- N16B which is the NCP for the channel attached 3705
...
Start the remote 3705 (Both chan adap must be offline!)
$ ./BIN/i3705 3705r-128k.cnf       
Note the addition "r" in the name of the config file.
...
Trunk: Line 2 connection has been established
```
The trunk line is now established between the local and remote 3705 running on host efoxcc1 and the 3705 running on host efoxcc2. Both 3705's use line 20 (this must be specified in the NCP as well).  Note the -fdx switch. This is FUll Duplex, which is mandatory for a connection to the remote 3705 (This is how we configured the loader program).    
...
EF & HJS (C)2025
ill appear after the relevant check.  
